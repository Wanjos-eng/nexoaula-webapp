...
test("publicação, inscrição e histórico persistidos entre tutor e alunos reais", async ({ browser }, info) => {
  ...
  try {
    ...
    await student.getByRole("link", { name: "Ver detalhes" }).click();
    await student.waitForURL(/\/sessoes\/[0-9a-f-]+$/);
    const detailUrl = student.url(), sessionId = new URL(detailUrl).pathname.split("/").pop();
    await student.getByRole("button", { name: "Simular Inscrição" }).click();
    await student.getByRole("button", { name: "Confirmar", exact: true }).click();
    const receipt = student.getByRole("status", { name: "Recibo de inscrição simulada" });
    ...
    await other.reload();
    await expect(other.getByRole("button", { name: "Simular Inscrição" })).toBeEnabled();
    await student.getByRole("button", { name: "Inscrever-se novamente" }).click();
    await student.getByRole("button", { name: "Confirmar", exact: true }).click();
