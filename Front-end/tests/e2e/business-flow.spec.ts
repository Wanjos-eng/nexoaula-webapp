...
test("publicação, inscrição e histórico persistidos entre tutor e alunos reais", async ({ browser }, info) => {
  ...
  try {
    ...
    const checkout = student.getByRole("dialog");
    await expect(checkout).toContainText("Valor da sessão");
    await expect(checkout).toContainText("R$ 25,00");
    await expect(checkout).toContainText("Comissão NexoAula (15%)");
    await expect(checkout).toContainText("R$ 3,75");
    await expect(checkout).toContainText("Tutor receberia");
    await expect(checkout).toContainText("R$ 21,25");
    await expect(checkout).toContainText("Total demonstrativo");
    ...
    await expect(receipt).toContainText("Comissão NexoAula (15%)");
    await expect(receipt).toContainText("R$ 3,75");
    await expect(receipt).toContainText("Líquido do tutor");
    await expect(receipt).toContainText("R$ 21,25");
    await expect(receipt).toContainText("Simulada · concluída");
