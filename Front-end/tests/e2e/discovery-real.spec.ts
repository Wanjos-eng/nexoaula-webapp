...
test("descoberta acadêmica persiste assuntos e respeita visibilidade entre usuários", async ({ browser }) => {
  ...
  try {
    ...
    await owner.getByLabel("Nome do grupo *").fill(groupName);
    ...
    await owner.getByRole("button", { name: "Configurar grupo", exact: true }).click();
    ...
    await reader.getByRole("button", { name: "Buscar grupos", exact: true }).click();
    ...
    await reader.getByRole("button", { name: "Buscar grupos", exact: true }).click();
    await expect(reader.getByRole("heading", { name: "Nenhum grupo encontrado" })).toBeVisible();
